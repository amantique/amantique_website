...
const upcomingEvents = [
  ...
  // Ajoute ici d'autres événements "À VENIR"
];
...
const ContactPage = () => {
  ...
  return (
    <div className="w-full text-center bg-cover bg-center bg-no-repeat text-[#0A0A0A] pb-4" style={{ backgroundImage: "url('/img/bg/bg_bio.jpg')" }}>
      <h2 className="text-7xl text-white font-extrabold pt-4 sm:text-9xl drop-shadow-lg">DATES</h2>
      
      {/* Section À VENIR */}
      <h3 className="text-left text-white text-5xl pl-4 sm:pl-8 md:pl-16 mb-4">À VENIR</h3>
        ...
        {upcomingEvents.slice(0, visibleUpcoming).map((event, index) => (
          <a key={index} href={event.link} target="_blank" rel="noopener noreferrer">
            <div className="relative w-full max-w-xs mx-auto group">
              <Image src={event.image} alt="Event Poster" width={320} height={320} className="object-cover" />
              ...
              {event.label && <div className="text-[#F20D01] absolute top-[-10px] left-0 w-full text-center text-5xl font-bold animate-bounce">{event.label}</div>}
            </div>
          </a>
        ))}
      </div>
      {visibleUpcoming < upcomingEvents.length && (
        <button onClick={() => setVisibleUpcoming(visibleUpcoming + 4)} className="mt-4 px-6 py-2 bg-[#F20D01] text-white font-bold rounded">Afficher plus</button>
      )}

      {/* Section TROP TARD... */}
      <h3 className="text-left text-white text-5xl pl-4 sm:pl-8 md:pl-16 my-4">TROP TARD...</h3>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 px-4">
      ...
      {visiblePast < pastEvents.length && (
        <button onClick={() => setVisiblePast(visiblePast + 4)} className="mt-4 px-6 py-2 bg-[#F20D01] text-white font-bold rounded">Afficher plus</button>
      )}
    </div>
  );
};
